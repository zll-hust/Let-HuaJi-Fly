...
//        dy = speed * Math.sin(Math.toRadians(angle));
    ...
    public static Role createNewMissile(int id, Player p, GUI gui) {
        if (random.nextDouble() < 0.25) {
            return new Missile(random.nextInt(gui.graphWidth), 0, id, gui, p);
        } else if (random.nextDouble() > 0.25 && random.nextDouble() < 0.5) {
            return new Missile(gui.graphWidth, random.nextInt(gui.graphHeight), id, gui, p);
            ...
            return new Missile(0, random.nextInt(gui.graphHeight), id, gui, p);
        }
    }
}
